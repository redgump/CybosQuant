...
1. **32비트 Python** (이 PC 의 기본 Python 3.12 는 64비트라 사용 불가.
   이 PC 검증 완료 경로: `C:\Program Files (x86)\Python314-32\python.exe`)
...
4. **CYBOS Plus 와 Python 스크립트의 권한(UAC) 레벨이 일치**해야 한다.
   ...
   이 PC 의 CYBOS Plus(`CpStart.exe`)는 매니페스트상 **항상 관리자 권한으로
   실행**되므로, **스크립트도 반드시 관리자 권한 터미널에서 실행**해야 한다.
   (관리자 PowerShell → `& "C:\Program Files (x86)\Python314-32\python.exe" collect_minutes.py`)
...
> ```powershell
> Set-Alias py32 "C:\Program Files (x86)\Python314-32\python.exe"
> ```
